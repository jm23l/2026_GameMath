...
public class TurnBasedGame : MonoBehaviour
{
    ...
    [SerializeField] TMP_Text rareChanceText;
    [SerializeField] TMP_Text enemyCountText;
    [SerializeField] TMP_Text rewardText;
    [SerializeField] TMP_Text rareItemText;
    ...
    public void StartSimulation()
    {
        ...
        rareChanceText.text = $"레어 아이템 {turn} 턴에 획득";
    ...
    void SimulateTurn()
    {
        ...
        for (int i = 0; i < enemyCount; i++)
        {
            ...
            if (totalDamage >= enemyHP)
            {
                ...
                if (reward == "Weapon" && Random.value < Mathf.Clamp01(0.2f + turn * 0.05f))
                {
                    rareItemObtained = true;
                    rareItemText.text = "레어 무기 획득";
                }
                else if (reward == "Armor" && Random.value < Mathf.Clamp01(0.2f + turn * 0.05f))
                {
                    rareItemObtained = true;
                    rareItemText.text = "레어 방어구 획득";
                }
            }
        }
    }
